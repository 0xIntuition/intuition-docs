...
The Intuition Smart Contracts utilize a configuration system that allows for dynamic adjustment of various parameters to optimize performance and security. This pages provides an overview of the configuration settings and fee structures used in the Intuition protocol on the **mainnet** deployment.

## MultiVault Configuration
...
When depositing or withdrawing into an atom or triple, a minimum deposit is required:   

- **Minimum Deposit:** 0.01 TRUST (1e16)

The percentage-based fees are applied to the total assets deposited or withdrawn after accounting for fixed costs:
...
## TrustBonding & Emissions Controller Configuration

The TrustBonding and Emissions Controller contracts manage the bonding of TRUST tokens and the emission schedule for new tokens.
...
These parameters are fixed and cannot be changed by governance.