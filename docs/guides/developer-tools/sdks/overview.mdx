...
import { base, baseSepolia, mainnet, sepolia, arbitrum, arbitrumSepolia } from 'viem/chains'

// Public client for read operations (Base Mainnet)
...
**Note**: The Intuition L3 network (Caldera rollup) is currently in development. Access the [Intuition Hub](https://intuition-testnet.hub.caldera.xyz/) for L3 network development tools and testing.
...
```typescript
import { getAtom, getTriple, pinThing } from '@0xintuition/sdk'
...
const data = await createAtomFromIpfsUpload(
  {
    ...
    pinataApiJWT: 'your-pinata-api-jwt',
  },
  {
    url: 'https://www.intuition.systems/',
    name: 'Intuition',
...
### Create from structured metadata (upload to IPFS)

Create an atom by uploading metadata to IPFS in one step:

```typescript
import { createAtomFromIpfsUpload, getEthMultiVaultAddressFromChainId } from '@0xintuition/sdk'

const ethMultiVaultAddress = getEthMultiVaultAddressFromChainId(walletClient.chain.id)
const data = await createAtomFromIpfsUpload(
  { walletClient, publicClient, address: ethMultiVaultAddress, pinataApiJWT: 'your-pinata-api-jwt' },
  {
    url: 'https://www.intuition.systems/',
    name: 'Intuition',
    description: 'A decentralized trust protocol',
...
<!-- Removed: createEthereumAccount (not available in SDK). Use createAtomFromSmartContract for contract addresses. -->

### Create a Smart Contract

Create an atom representing a smart contract:
